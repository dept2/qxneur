...
class X11Kbd: public QAbstractNativeEventFilter
{
  public:
    ...
    bool init()
    {
      ...
      {
        qWarning() << "Couldn't connect to X server: error code" << (m_connection ? xcb_connection_has_error(m_connection) : -1);
        return false;
      }
      ...
      if ((event->response_type & ~0x80) == m_eventType)
      {
        ...
        switch(sevent->xkbType)
        {
          case XCB_XKB_STATE_NOTIFY:
            xkb_state_update_mask(m_state, sevent->baseMods, sevent->latchedMods, sevent->lockedMods,
                                                   sevent->baseGroup, sevent->latchedGroup, sevent->lockedGroup);
    ...
    void readKbdInfo(KbdInfo& info) const
    {
      ...
      for(xkb_layout_index_t i = 0; i < count; ++i)
      {
        ...
        info.append({linfo.syn, linfo.name, linfo.variant, i});
  ...
  private:
    ...
    void readState()
    {
      if (m_keymap)
        xkb_keymap_unref(m_keymap);
      m_keymap = xkb_x11_keymap_new_from_device(m_context, m_connection, m_deviceId, XKB_KEYMAP_COMPILE_NO_FLAGS);
...
void X11Kbd::readKbdInfo()
{
  ...
  if (_groups.isEmpty())
  {
    for (auto group : info.groups())
      _groups.append(QXNLanguage::layoutNameToLanguage(group.name));
  }

  _currentLayout = QXNLanguage::layoutNameToLanguage(info.currentName());
}
...
void X11Kbd::setGroup(const QString& group)
{
  for (int i = 0; i < _groups.size(); ++i)
  {
    QXNLanguage::Language language = _groups.at(i);
    if (QXNLanguage::languageToText(language).contains(group))
    {
      lockGroup(uint(i));
      break;
    }
...
QList<QXNLanguage::Language> X11Kbd::groups() const
{
  return _groups;
}


QXNLanguage::Language X11Kbd::currentLayout() const
{
  return _currentLayout;
}


void X11Kbd::nextGroup()
{
  int i;
  for (i = 0; i < _groups.size(); ++i)
  {
    if (_groups.at(i) == _currentLayout)
      break;
  }

  int newGroup = i + 1;
  if (newGroup > _groups.size() - 1)
    newGroup = 0;

  lockGroup(uint(newGroup));
}
